...
public class Prueba_Personas extends JFrame {
    private List<Persona> personas = new ArrayList<>();
    List<Persona> personasPruebas = new ArrayList<>();
    DefaultTableModel model = new DefaultTableModel();

    //Se declara el constructor del formulario
    public Prueba_Personas(){
        initComponents();
        obtenerDatos();
    ...
    private void  obtenerDatos() {
        ...
        personas.add(new Persona(1, "María", "González", 2013, 2, 5.67));
...
// metodos de los com ponentes para inicializar el formulario
    private void initComponents() {
        JPanel panel = new JPanel(new BorderLayout());
        JPanel panelBotones = new JPanel(new FlowLayout());

        JButton btnNumeroPersonas = new JButton("Ver el número de personas");
        btnNumeroPersonas.addActionListener(e -> {
            long numeroPersonas = personas.stream()
                    .count();
            JOptionPane.showMessageDialog(this, "Número de personas Totales: "+ numeroPersonas);
        });
        panelBotones.add(btnNumeroPersonas);

        // suma de salarios
        JButton btnSumaTodosSalarios = new JButton("Suma de todos"+
                "los salarios");
        btnSumaTodosSalarios.addActionListener(e -> {
            double sumaSalarios = personas.stream() //stream<persona>
        ...
        //Numero de personas con un salario mayor a 500
        JButton btnPersonasSalarioMayor500 = new JButton("Cantidad de "+
                "personascon un salario mayor a 5000: ");
        btnPersonasSalarioMayor500.addActionListener(e -> {
            long cantidadMayor500 = personas.stream()
                    .filter(persona -> persona.getSalario() >500)
                    .count();
            JOptionPane.showMessageDialog(this, "Cantidad de personas con un "+
                    "salario mayor a 500: " + cantidadMayor500);
        });
        panelBotones.add(btnPersonasSalarioMayor500);

        JButton btnSalarioMayorRobles = new JButton("Mayor salario de acuerdo a las "+
                "personas que se apellidan Robles");
        btnSalarioMayorRobles.addActionListener(e -> {
            double salarioMayor = personas.stream()
                    .filter(persona -> persona.getApellido().equals("Robles"))
                    .mapToDouble(persona -> persona.getSalario())
                    .max().getAsDouble();
            JOptionPane.showMessageDialog(this,"Salario mayor entre todas las personas " +
                    "que se apellidan Robles: "+ salarioMayor);
        });
        panelBotones.add(btnSalarioMayorRobles);

// personas que nacieron despues del 2000
        JButton btnPersonasDespues2000 = new JButton("<html>Personas que nacieron despues "+
                "del<br>año 2000</html>");
        btnPersonasDespues2000.addActionListener(e -> {
            personasPruebas = personas.stream()
                    .filter(persona -> persona.getAnioNacimiento() > 2000)
                    .collect(Collectors.toList());
            llenarTabla(personasPruebas);
            setTitle("Personas nacidas"+
                    "despues del año 2000");
        });
        panelBotones.add(btnPersonasDespues2000);

        //personas con coeficiente entre 5 y 10
JButton btnCoeficienteEntre5y10 = new JButton("Personas con coeficiente entre 5 y 10");
btnCoeficienteEntre5y10.addActionListener(e -> {
    personasPruebas = personas.stream()
            .filter(persona -> persona.getCoeficiente() >= 5 && persona.getCoeficiente() <=10)
            .collect(Collectors.toList());
    llenarTabla(personasPruebas);
    setTitle("Personas con coeficiente entre 5 y 10");
});
panelBotones.add(btnCoeficienteEntre5y10);

// Personas que tengan el apellido Robles
JButton btnApellidoRobles = new JButton("Personas que tengan el apellido Robles");
btnApellidoRobles.addActionListener(e -> {
    personasPruebas = personas.stream()
            .filter(persona -> persona.getApellido().equals("Robles"))
            .collect(Collectors.toList());
    llenarTabla(personasPruebas);
    setTitle("Personas que tengan el apellido Robles");
});
panelBotones.add(btnApellidoRobles);

// boton para mostrar a todas la lista de personas
JButton btnMostrarTodasLasPersonas = new JButton("Mostrar todas las personas");
btnMostrarTodasLasPersonas.addActionListener(e -> {
    llenarTabla(personas);
    setTitle("Personas existentes");
});
panelBotones.add(btnMostrarTodasLasPersonas);
...
// boton de salida
        ...
        // agregar el contenido de la tabla al centro del contenedor
    ...
    public static void main(String[] args){
        new Prueba_Personas().setVisible(true);
